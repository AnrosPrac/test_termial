...
async def notification_history(
    ...
    notifications = await db_instance.notifications.find({}, {"_id": 1}) \
        .skip(skip) \
        .limit(limit) \
        .sort("created_at", -1) \
        .to_list(length=limit)
...
async def export_payments(
    ...
    """
    return StreamingResponse(
        export_payments_csv(db_instance),
        media_type="text/csv",
...
async def export_tickets(
    ...
    """
    return StreamingResponse(
        export_tickets_csv(db_instance),
        media_type="text/csv",
...
async def get_audit_logs(
    ...
    """
    from app.admin.hardened_analytics import AuditService
    
    logs = await AuditService.get_recent_actions(
        db=db_instance, 
        limit=limit, 
        admin_email=admin_email
    )
    
    # Format BSON types for JSON response
    for log in logs:
        log["_id"] = str(log["_id"])
        if "timestamp" in log:
...
async def clear_system_cache(
    ...
    """
    Clear all in-memory analytics cache
    """
...
async def get_pricing_config(
    ...
    """
    Get current Gemini API pricing and tier configurations
    """
    from app.admin.hardened_analytics import PricingConfig
    
    return {
        "status": "success",
        "gemini_pricing_usd": PricingConfig.GEMINI_PRICING_USD,
        "conversion_rate": PricingConfig.USD_TO_INR,
        "valid_tiers": list(PricingConfig.VALID_TIERS)
    }